        ...
        .query(
            `
                WITH start_station_arrival AS(
                    SELECT station.id, station.arrival AS "startArrival"
                    FROM station, ticket
                    WHERE station.id = ticket.startStartion_id 
                ),
                end_station_arrival AS(
                    SELECT station.id, station.arrival AS "endArrival"
                    FROM station, ticket
                    WHERE station.id = ticket.endStation_id
                ),
                seats_booked AS(
                    SELECT ticket.startStartion_id, ticket.endStation_id, ticket.departureTime,
                ...
                seats_free AS(
                    ...
                    FROM journey, trainset, carriage, seat, seats_booked
                    WHERE journey.id = ?
